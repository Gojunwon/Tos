...
기존 커널에서의 방식 중 하나인 준비큐와 대기 상태 큐의 구현이 제대로 구현이 되지 않아 스케줄링의 방식을 선택 할 때의 어려움이 있었습니다. 큐의 구현을 하기 위해 기존에 하나만 있던 큐를 3개를 만들어서 독립적으로 만들어서 하여서 스케줄링에 따라 필요한 큐를 확보하여 구현을 성공하였습니다. 또한 큐를 실행하고 남아있는 프로세스들을 없애기 위해서는 재시작을 해야하는 어려움이 있었는데 리셋 명령어 `rs`를 만듬으로써 나가지 않아도 프로세스 처리를 할 수 있게끔 만들었습니다. 
## 현재 폴더 구조
