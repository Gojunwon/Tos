...
TOS는 미니 쉘 프로젝트입니다. 현재 단계의 목표는 완성된 커널 시뮬레이터가 아니라, 이후 운영체제 코어 시뮬레이터를 붙이기 위한 앞단 인터페이스를 만드는 것이며 프로세스 스케줄링의 종류에 따라 프로세스가 어떻게 처리가 되는지 사용자가 볼 수 있게 만들어주는 프로젝트입니다.

지금 구현된 기능은 사용자 입력을 받아 파싱하고, 내장 명령어는 쉘 내부에서 처리하며, 일반 외부 명령어는 `fork()` + `execvp()` + `waitpid()` 흐름으로 실행하는 기본 쉘입니다. 추가로 시뮬레이션용 PCB와 Ready Queue를 만들기 위한 `create`, `ps` 명령이 포함되어 있으며 I/O 입출력 시간을 포함하는 `I/O wait` , 스케줄링을 설정하는 `set_sched` *** (***은 스케줄링 종류) , 세팅한 값을 토대로 실행시키기 위한 `run` 명령어가 있습니다. 
사용자의 명령어 확인 편의성을 위해 `help`를 통해 현재 실행 가능한 명령어를 보여줄 수 있게 했습니다.

## 🚀 Key Features (핵심 구현 기능)

### 🖥️ 커널 및 프로세스 관리 (Kernel & Process Management)
* **독립적 멀티 큐(Multi-Queue) 아키텍처**
  * 준비 큐(Ready Queue)와 대기 상태 큐(Waiting Queue)를 완전히 독립된 내부 자료구조로 분리 구현.
  * 복잡한 스케줄링 알고리즘 전환 시 자원 간섭을 최소화하고 스케줄러 상태 동기화 정밀도 향상.
* **소프트 커널 리셋 시스템 (`rs` 기능)**
  * 프로그램 재시작 없이 내부 프로세스 테이블 초기화 및 메모리 해제를 수행하는 커널 동적 리셋 로직 설계.
  * 시뮬레이션 연속성을 보장하여 테스트 및 디버깅 효율성 극대화.
